		       ...
		       off_t offset, struct fuse_file_info *fi)
{
	...
	for (int i = offset; i < _MAX_DIR_NUM; i++) {
		...
		if (dir_ent.in_use) {
			if(filler(buf, dir_ent.file_name, NULL, i+1)) {
				return 0;
			}
		}
	}
...
// close (i.e., release), create, fgetattr, mknod (only for plain files), open, 
// read, readlink, rmdir, statfs, symlink, truncate, unlink, and write
...
//	struct DirectoryEntry parent_dir_ent = {};
//	if (read_DirectoryEntry(subpath, &parent_dir_ent) == -ENOENT) {
//		return -ENOENT;	
//	}
//
//	for (int i = 0; i < _MAX_DIR_NUM; i++) {
